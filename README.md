...
[실습] 노드 생성하기
기존의 대화 내용에 기반하여 GPT로 답변을 생성하는 노드 만들기
...
// 리스트에 문장 추가하기
response = graph.invoke({"messages": ["안녕하세요! 저는 비비입니다"]})
...
class BasicToolNode:
    ...
    def __init__(self, tools: list) -> None:    # 딕셔너리 형태로 도구 이름과 도구 자체를 저장
        self.tools_by_name = {tool.name: tool for tool in tools}

    def __call__(self, inputs: dict):    # input은 랭 그래프에서 상태 관리 state가 딕셔너리로 전달됨.
...
graph_builder.add_node("tools", tool_node) # BasicToolNode 클래스를 tools 노드로 graph_builder에 추가
```

[실습] 라우터 설정하기

- AI 에이전트가 랭그래프 내에서 스스로 다음 경로를 선택해야 할 때 라우터를 활용함
- 라우팅: 상황에 따라 방향을 결정하는 것
- 조건에 따라 활성화 or 비활성화되는 조건부 엣지를 사용
...
15-1 에이전트의 공통 목표 만들기
목표를  점검하는 비즈니스 분석가 에이전트: 사용자의 요구 사항과 작업의 진행 상황을 분석하여 현재 작업의 목표와 방법을 제시하는 역할 수행

[실습] 사용자의 의도를 파악하는 에이전트 business_analysist 만들기
...
============ BUSINESS ANALYST ============
[Business Analyst] ```
- 목표: HYBE와 JYP의 비교를 중심으로 CEO와 경영전략에 대해 다루는 책 작성
- 방법: 
  1. HYBE와 JYP의 CEO에 대한 정보 수집 및 분석
  2. 두 기업의 경영전략 비교 분석
  3. 관련 문헌 및 아티클 조사
  4. 목차 구성 및 각 장별 내용 작성
  5. 초안 작성 후 리뷰 및 수정
============ SUPERVISOR ============
[Supervisor] agent='web_search_agent' done=False description='사용자가 요구한 HYBE와 JYP의 CEO와 경영전략 비교를 중심으로 책을 쓰기 위해 필요한 정보를 웹에서 수집하여 목차 작성을 시작한다.' done_at=''

============ WEB SEARCH AGENT ============
-------- web search -------- {'name': 'web_search', 'args': {'query': 'HYBE CEO 2025'}, 'id': 'call_X74iQxyTx1S7wBWf10AsWD8g', 'type': 'tool_call'}
```
...
```python
1. 목표 정의: 어떤 의도로 목차를 작성할 것인지 결정
2. 간략한 목차 작성: 간단한 목차 작성
3. 상세한 목차 작성: 목차를 상세하게 만듦
```

[실습] 목차 작성을 위한 템플릿 만들기
...
[실습] 목차 작성 템플릿을 활용해 시스템 프롬프트 발전시키기
...
사용자에게 다음 작업을 물어보기 전에 비즌스 분석가 에이전트가 현재 상태를 파악하고,
확인이 필요한 경우에만 사용자에게 문의하도록 시스템 수정

![book_writer_0.png](attachment:25c09cc4-f09a-4180-b646-9fb840b7223b:book_writer_0.png)

[실습] 스스로 판단하고 작업하는 멀티에이전트 시스템 만들기
...
```python
# 벡터 검색 결과가 만족스럽지 않을 때 웹 검색하도록 수정
def supervisor(state: State): # supervisor 에이전트 추가
    print("\n\n============ SUPERVISOR ============")
...
```
User    : HYBE와 JYP의 CEO, 경영 전략, 기업 문화에 대해 비교하는 내용으로 목차 써줘

============ BUSINESS ANALYST ============
[Business Analyst] ```
- 목표: HYBE와 JYP의 CEO, 경영 전략, 기업 문화에 대한 비교 내용을 담은 목차 작성
- 방법: HYBE와 JYP의 기업 정보를 조사하여 각 CEO의 경영 스타일, 두 기업의 경영 전략 및 기업 문화를 비교 분석한 후 이를 바탕으로 목차 초안을 작성
```

============ SUPERVISOR ============
[Supervisor] agent='web_search_agent' done=False description='현재까지 확보된 정보로 목차 작성을 위한 충분한 데이터가 없는 상황입니다. 따라서 웹 검색 에이전트를 사용하여 HYBE와 JYP의 최신 정보, CEO의 경영 전략 및 기업 문화를 조사하여 목차 작성을 위한 자료를 추가로 수집해야 합니다.' done_at=''
```
...
![book_writer_1.png](attachment:f782f066-e646-4e8a-a5ab-e83de263625f:book_writer_1.png)

[실습] 목차 조언 항목 추가하고 business_analyst에 반영하기
...
[실습] 목차를 검토하는 outline_reviewer 만들기
...
[실습] 벡터 검색 에이전트도 비즈니스 분석가 에이전트에게 조언하도록 구성하기
...
16-1 로컬에서 라마와 임베딩 모델 사용하기

[실습] 라마 기반으로 간단한 챗봇 만들기
...
빠른 답변 vs 사용자가 원하는 답변

- 상황을 고려해서 개발하면 됨
    - 게임에서의 대화일 경우 2-3초만 지연되도 어색하고 긴 문장을 만들면 부자연스러움
    - 보고서 작성의 경우에는 시간이 걸려도 정확하고 구조화된 답변을 제공해야 함
        - 랭 그래프로 사용자가 원하는 방식으로 작업하도록 유도해야함

기능 개발이 끝났으면 그때부터 시작이다

- 언어 모델의 환각 현상 문제 해결을 위해 RAG 기술 활용하거나 웹 검색 기능 추가, 랭 그래프 사용해 답변 검토하는 방법 등 여러 기술들을 적절히 조합해서 더 나은 성능을 발휘하는 AI 에이전트로 발전시켜야함
